...
| r011 | （补正轮）欠账补正 + 三处授权例外（满员自动拒待批申请 / 邀请码入口与未登录闭环 / worker 健康上报） | `r011-debt-backfill.md` | **阶段 1 文档完成，待你批**（2026-09-20） | `docs/rounds/r011-debt-backfill/`（含 design / manual-verification / changes / review 骨架） | 实现后由人打 `round-r011-done`（cp tag `cp-r011-*`） |
