...
| r011 | （补正轮）欠账补正 + 四处授权例外（满员自动拒待批申请 / 邀请码入口与未登录闭环 / worker 健康上报 / **焦点规则收窄：说话不获焦点**） | `r011-debt-backfill.md` | **实现完成，待合并**（2026-09-20：cp-1~cp-8 全落，pytest 164 / smoke 47-47 / tsc · build 绿；人工剧本 MV-1~MV-12 待你跑） | `docs/rounds/r011-debt-backfill/`（含 design / redirect-02 / **redirect-03** / manual-verification / changes / review 定稿） | 合并后由人打 `round-r011-done`（cp tag `cp-r011-1/1b/2/3/4/5/6/6b/7/8`） |
| r013 | （修+增量混合轮）**演示就绪**：三人档焦点回归门禁 · 强停共享取证 · 转写 worker 自愈与可观测 · 房间结束后的只读回看页 · 全服大屏并入交流页抽屉；另附**断线重连真实中断级验收方案**（只出方案） | `r013-demo-readiness.md` | **实现完成，待合并**（2026-09-20：cp-1~cp-7 全落；pytest **212** / smoke **59-59** / tsc·build 绿 / 五份真机脚本 30-18-17-8-0.5s） | `docs/rounds/r013-demo-readiness/`（design / changes / review） | 合并顺序 `r011 → r012 → r013`（cp tag `cp-r013-1…7`） |
| r012 | （功能轮）超管用户（隐身进任意房 + 任一房房主能力 + 不计入人数 + **只管理不发布音视频**）+ 简单管理后台（房间/用户/纪要列表 + 结束/删除/重生纪要 + 审计）+ 全服大屏聊天（右侧 Copilot 式面板 + SSE 通知通道） | `r012-superadmin-console.md` | **实现完成，待你复核合并**（2026-09-20：cp-1~cp-8c 全落；收工时全量 `pytest` **212** / `smoke` **59-59** / `tsc`·`build` 绿；未闭合 ①双浏览器隐身交叉验证 ②reduced-motion 强制模拟——两条均已由 cp-7b 闭合；遗留见 `review.md` §6） | `docs/rounds/r012-superadmin-console/`（含 design / changes / review 骨架，`redirect-01` 登记单） | 合并后由人打 `round-r012-done`（cp tag `cp-r012-1/1b/1c/2/4/4b/5/6/7/7b/8/8b/8c`；**无 cp-3**，其内容并入 cp-4 的 `6051756`，见 r012 changes 台账补正） |
...
> **r011 → r012 的交接（2026-09-20）**：r012 自 r011 的 HEAD 开出，并已把 r011 的收尾两个提交（cp-8 / cp-8b）**merge 进 r012 分支**，故 r012 分支**含 r011 全部提交**；合并顺序 = `r011 → r012`（先 r011 后 r012，或直接合 r012 也会把 r011 一并带上）。|
