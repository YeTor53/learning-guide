...
| r012 | （功能轮）超管用户（隐身进任意房 + 任一房房主能力 + 不计入人数 + **只管理不发布音视频**）+ 简单管理后台（房间/用户/纪要列表 + 结束/删除/重生纪要 + 审计）+ 全服大屏聊天（右侧 Copilot 式面板 + SSE 通知通道） | `r012-superadmin-console.md` | **实现完成，待你复核合并**（2026-09-20：cp-1~cp-7 全落，`pytest` 200 passed / `tsc`·`build` 绿 / 真机取证见 `review.md`；未闭合 ①双浏览器隐身交叉验证 ②reduced-motion 强制模拟） | `docs/rounds/r012-superadmin-console/`（含 design / changes / review 骨架，`redirect-01` 登记单） | 合并后由人打 `round-r012-done`（cp tag `cp-r012-1/1b/1c/2/3/4/4b/5/6/7`） |
