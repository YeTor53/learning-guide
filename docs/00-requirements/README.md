...
| r012 | （功能轮）超管用户（隐身进任意房 + 任一房房主能力 + 不计入人数）+ 简单管理后台（房间/用户/纪要列表 + 结束/删除/重生纪要 + 审计）+ 全服大屏聊天（SSE 通知通道） | `r012-superadmin-console.md` | **阶段 1 文档完成，待你批**（2026-09-20：`ASK-r012-1` 的 Q1~Q18 待答；未批不动代码） | `docs/rounds/r012-superadmin-console/`（含 design / changes / review 骨架，`redirect-01` 登记单） | 实现后由人打 `round-r012-done`（cp tag `cp-r012-1..7`） |

> **合并顺序（2026-09-20 r009.5 登记）**：`r005 → r006 → r007 → r008 → r009 → r010 → r009.5`。r005~r010 与 r009.5 于 **2026-09-20 全部合入 `main` = `96b1153`**（`round-r005-done` … `round-r010-done`、`round-r009.5-done` 齐）；r011 从该 HEAD 开出，本轮无前置未合并轮次。**r012 自 r011 的分支 HEAD（`ab8edb5`，cp-7）开出**，故合并顺序 = `r011 → r012`（r012 分支已含 r011 全部提交）。|