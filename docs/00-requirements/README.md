...
| r012 | （功能轮）超管用户（隐身进任意房 + 任一房房主能力 + 不计入人数 + **只管理不发布音视频**）+ 简单管理后台（房间/用户/纪要列表 + 结束/删除/重生纪要 + 审计）+ 全服大屏聊天（右侧 Copilot 式面板 + SSE 通知通道） | `r012-superadmin-console.md` | **阶段 1 已批，待放行实现**（2026-09-20：`ASK-r012-1` 的 Q1~Q18 已答并落地，见需求单 §10.1；未批不动代码） | `docs/rounds/r012-superadmin-console/`（含 design / changes / review 骨架，`redirect-01` 登记单） | 实现后由人打 `round-r012-done`（cp tag `cp-r012-1..7`） |
