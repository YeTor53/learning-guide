...
| r010 | （实现轮）语音转文字并入讨论流 + STT 获取方案 | `r010-transcription.md` | **实现中（2026-09-20）**：cp-1 已落（迁移 009 + `services/stt.py` 唯一出口 + `repositories/transcripts.py` + 上传/列表路由 + 15 条用例；含 CR-01 依赖登记） | `docs/rounds/r010-transcription/`（含 `cr-01.md`） | 合并后由人打 `round-r010-done`（cp tag `cp-r010-0/1`） |
| r009.5 | （补正轮）r008/r009 欠账补正：台账 · tag · 审查回填 · 纠错 · 取证 · 教学页 | `r009.5-debt-backfill.md` | **实现完成，待合并**（cp-1~cp-5 全落，2026-09-20）；分支 `req/r009.5-debt-backfill` | `docs/rounds/r009.5-debt-backfill/`（review 已定稿） | 合并后由人打 `round-r009.5-done`（cp tag `cp-r009.5-1..5`） |
