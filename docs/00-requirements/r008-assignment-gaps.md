...
| R3 | ~~语音转文字~~（移至 r009）：房内把发言转成文字（本端麦克风分段 → 服务端 STT → 落库），文本进纪要输入；界面可看片段 | 加分「旁路录音转写后再生成纪要」 | 无 |
...
- **语音转文字（整体移至 r009）**：你的新口径 = **默认开启** + 说的话**并入文字对话** + 连成员进出等管理信息一起构成讨论记录；属数据模型级改动，故下一轮做，并含「STT 获取方案」设计（见 `docs/rounds/r009-transcription/redirect-01.md`）。
...
| cp-2 | 纪要前端（纪要页 + 卡片入口） |
| cp-3 | 限时邀请（后端 + 前端 + 用例） |
| cp-4 | ~~语音转文字~~ → 移至 r009（见 `docs/rounds/r009-transcription/redirect-01.md`） |
...
| 日期 | 版本 | 改了什么 | 依据 |
| --- | --- | --- | --- |
| 2026-09-19 | v2 | **口径调整**：邀请有效期改「最长 1 分钟」；语音转文字整体移至 r009（默认开启 + 并入对话 + 含管理信息 = 难点大模块），STT 获取方案列为下一轮工作 | 你 2026-09-19 的批复 |
