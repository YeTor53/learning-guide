...
| R3 | ~~语音转文字~~（移至 r010）：房内把发言转成文字（本端麦克风分段 → 服务端 STT → 落库），文本进纪要输入；界面可看片段 | 加分「旁路录音转写后再生成纪要」 | 无 |

## 2. 口径与边界（读back）

- **纪要**：输入 = 房间主题/标题/简介 + 在册成员（角色/是否在场）+ 近期聊天与系统消息 + **转写文本（有则带）**；输出 markdown 正文；一间房一份、可覆盖重生；**不在结束瞬间自动跑**（结束页给按钮），避免外部调用失败污染结束流程。
- **邀请**：Host/Moderator 可生成；有效期**最长 1 分钟**（默认 60 秒，可选 30/60 秒；上限单点可调 `INVITE_TTL_MAX_SECONDS`）、默认 **1 次**（可改 1~50）；用码加入 = **直接成为在册成员**（跳过等候室），仍受 8 人上限（满员 409 `ROOM_FULL`）；复制链接 `/join?code=XXXX`。
- **语音转文字（整体移至 r010）**：你的新口径 = **默认开启** + 说的话**并入文字对话** + 连成员进出等管理信息一起构成讨论记录；属数据模型级改动，故下一轮做，并含「STT 获取方案」设计（见 `docs/rounds/r010-transcription/redirect-01.md`）。**（r009.5 纠错：原定 r009，2026-09-19 焦点系统插入后顺延为 r010）**
...
| cp-2 | 纪要前端（纪要页 + 卡片入口） |
| cp-3 | 限时邀请（后端 + 前端 + 用例） |
| cp-4 | ~~语音转文字~~ → 移至 r010（见 `docs/rounds/r010-transcription/redirect-01.md`；原定 r009，2026-09-19 顺延） |
...
| 日期 | 版本 | 改了什么 | 依据 |
| --- | --- | --- | --- |
| 2026-09-19 | v2 | **口径调整**：邀请有效期改「最长 1 分钟」；语音转文字整体移至 r010（原定 r009；默认开启 + 并入对话 + 含管理信息 = 难点大模块），STT 获取方案列为下一轮工作 | 你 2026-09-19 的批复 |
| 2026-09-19 | v1 | 建页：三件需求 + 口径 + E1~E12 + cp 切分 + 密钥需求 | 你 `Q1=1` + 「再加一个语言转文字需求」 |
| 2026-09-20 | v3 | **轮次号纠错**：转写轮次 r009 → **r010**（4 处）+ 死链路径改 `r010-transcription/redirect-01.md` | r009.5（`req/r009.5-debt-backfill`，你 2026-09-20「用一轮.5修复」） |
