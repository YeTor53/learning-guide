...
- **r001 首页筛选工具栏「太下了」**（hero 高 498px 把筛选条顶到首屏之外；2026-09-18 实测）：**本轮不修**，已登记 `global-roadmap.md` §9 遗留台账待你拍板（本轮只做实时房间，顺手改首页会扩大范围）。
...
| cp-r002-3 | **前端**：房内页与 4 个 `live` 组件 + 4 个 hooks（含连接状态机 `useRoomConnection` 与设备状态 `useLocalDeviceState`）+ `api/livekit.ts` + 路由与详情页入口 + 样式（沿用设计令牌）+ 连接状态徽标与「正在重连…」态 | `npx tsc --noEmit` 全绿；四态与重连态齐全；断网重连人工验一次并把 C-3（重连后轨道恢复行为）实测结论回填实现页；靠用户已开的 dev 服务热更新肉眼验收（不另起端口、不反复 build） |
