...
| Q5 | 离开 / 断线语义 | 只处理「显式离开」「被踢」「房间结束」；关标签页/断网**不改库**（侧栏显示离线） | 自动判定需 Webhook 公网回调，留 M5 |
...
| Q8 | 验收方式 | 自动化覆盖 Token 契约 + 权限拦截 + 外部失败分支（打桩）；实时行为（声画互通、满员拒绝、真断开）列人工双浏览器验收 | 只能真跑的行为不假装自动化 |
...
- [ ] 断开归因：被移出 → 「你已被移出房间」；结束 → 「房间已结束」；网络 → 「连接已断开」+ 重连
...
| 演示环境无外网 / 现场网络抖动 | 保留自建 `livekit-server --dev` 降级路径（ADR-0001 + §8 步骤）；失败面已隔离，其余功能不受影响 |
...
| cp-r002-3 | **前端**：房内页与 4 个 `live` 组件 + 3 个 hooks + `api/livekit.ts` + 路由与详情页入口 + 样式（沿用设计令牌） | `npx tsc --noEmit` 全绿；四态齐全；靠用户已开的 dev 服务热更新肉眼验收（不另起端口、不反复 build） |
| cp-r002-4 | **冒烟 + 教学页 + 收官回填**：`smoke.py` 四步、`docs/tutorials/r002-livekit-demo.md`、`docs/tutorials/r002-livekit-dev-guide.md`、README/AGENTS/roadmap 回填、需求单勾选与证据表 | `smoke.py` `PASS n/n`；教学页示例实跑；矩阵无 `planned`；`git status` 干净 |
