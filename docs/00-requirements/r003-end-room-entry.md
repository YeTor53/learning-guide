...
- [ ] V1 房主视角：控制坞右侧是危险色「结束房间」且**可点**（原「离开」不再出现）—— 截图 + `document.querySelector('.live-ctrl-end')` 实测
- [ ] V2 点击出确认框，`Esc` 可取消、取消后房间与连接均无变化 —— 截图 + 实测
- [ ] V3 确认后：房主被断开并回到房间列表，该卡片状态「已结束」且无动作 —— 截图
- [ ] V4 另一浏览器（同房间成员）3 秒内断开并显示「房间已结束，仅可查看历史内容」 —— 截图 + 时间
- [ ] V5 非房主视角按钮仍是「离开」，行为不变（离开=回列表、需重新申请才能再进） —— 截图
- [ ] V6 库侧三件事 + 实时断开：`rooms.status='ended'`、成员 `inactive/room_ended`、`pending` 申请 `cancelled`；接口返回 `livekitApplied` 实测值 —— SQL 输出 + 响应体
- [ ] V7 直接调接口的越权路径：非房主调 `POST /rooms/{id}/end` → 403 `FORBIDDEN`（后端未改，取值取证即可）
- [ ] V8 质量门禁：`npx tsc --noEmit` 全绿、`npm run build` 成功、`pytest backend/tests -q` 仍 **95 passed**（后端零改动）
- [ ] V9 零 emoji / 单一图标库（Lucide）扫描无新增命中
- [ ] V10 文档对账：功能页 §4.2 按钮矩阵、§4.5 防呆条④、§4.9、§6 第 10 步；`r002-livekit.md` 变更记录；风格指南令牌表与 §12.1 离场段；`tutorials/r002-livekit-demo.md` 第 9 步与 `setup` 第 11 行 —— 全部与代码一致且示例实跑过
- [ ] V11 r002 收官回填四项齐（review.md 无「待核/待实现后填」残留、需求单 `closed`、索引表 r002 行已改、roadmap 三处矛盾已消）
...
| `cp-r003-3` | 真机取证 + 教学页实跑 + 审查报告 | V1~V11 逐条有证据 |
