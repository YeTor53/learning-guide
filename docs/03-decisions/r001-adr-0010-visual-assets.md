...
3. **放置与动效**（2026-09-18 按用户要求修订）：`.hero-thinker` 绝对定位在大标题右侧，宽 `clamp(262px, 28.75vw, 437px)`（**比首版大 25%**，`right: 0` 以避免横向溢出），`pointer-events: none`，`z-index` 低于正文。
   位移规则（2026-09-18 二次修订，虚拟上下界）：
   - **虚拟上界 = 0**：页面在顶部时图版在最高位；
   - 向下滚动时按滚动量的 **10%** 向下移动；
   - **虚拟下界 = 图版自身高度 × 35%（并夹在 90–200px 之间）**：与页面布局无关，任何页面长短、窗口大小下都保有可用行程（此前的实现把下界绑在 `hero 底边 − 图版底边` 上，页面一变就可能算成 0，视差直接失效）；
   - **允许被遮挡**：图版外层是裁剪容器 `.hero-thinker-clip`，其下边延伸到**房间列表区顶线**（`.hero` 底边 + 其 `var(--s-6)` 下边距），越过该线的部分被裁掉，视觉上像滑到列表区下面；保证图版永远不会压住列表内容。
   - rAF 合帧、滚动监听 passive、`prefers-reduced-motion: reduce` 时完全不绑定滚动（静止在最高位）。
