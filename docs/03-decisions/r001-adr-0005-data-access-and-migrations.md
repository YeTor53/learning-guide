...
前置已定：P5=PostgreSQL、P6=Python 后端、P9=本机安装 PostgreSQL。约束：交付要求要求「可运行的 SQL Schema + 迁移或初始化脚本」，交付需可复核，面试会追问取舍，单人 16 小时预算。
...
| --- | --- | --- | --- | --- |
| 交付要求「SQL 设计」是否显性 | SQL 就是交付物本身 | SQL 被表达式包裹，需额外交 DDL | 需额外交 DDL（或从模型导出） | **A/D** |
...
| B/C | 类型安全、迁移成熟、CRUD 快 | 多两层概念与依赖；SQL 被包起来，反而要额外交一份 DDL 才能满足交付要求；本项目的锁/部分唯一索引/批量 UPDATE 是 ORM 最不擅长的部分 |
...
1. 交付要求明确要「可运行的 SQL Schema」，手写 SQL 让交付物本身就是答案，不需要额外导出 DDL；
...
- 结论：**P10 的决策依据是 ADR-0005 §2 的那些维度**（交付要求显性、可审查、精细 SQL 表达力、依赖、面试叙事），不是速度。
...
- 交付要求只要求「可运行的 SQL Schema + **迁移或初始化脚本**」，不要求回滚；我们也没有多环境 —— 所以 D 的收益在本项目里落不到实处。
