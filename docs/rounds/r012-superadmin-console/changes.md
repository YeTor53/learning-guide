...
| 日期 | 轮次 | 变更 | 依据 |
| --- | --- | --- | --- |
| 2026-09-20 | r013 `cp-6` | 大屏在**交流页**的入口由「不挂」改为「右抽屉第四个 tab」（`GlobalChatPanel` 拆分 + `RoomSidePanel` 第 4 tab）；常规页顶栏抽屉壳与 r012 行为不变 | r013 需求单 §1E；你 2026-09-20「把到 6 的都做了」 |
...
| 用例隔离 · 审计表（cp-8c） | 我在真机上用超管删了一个测试房 → `admin_audit` 多一条 `room.delete` → `test_admin_api.py` 两条「审计起始为空」用例失败（2 failed / 200 passed）。属**用例对共享开发库不设防**（与 cp-7 修的 `global_messages` 同类） | 修：`test_admin_api.py` 加 autouse 夹具，在 `db` 的 force_rollback 事务里 `DELETE FROM admin_audit`（不提交、不动真机数据）。复跑：单文件 9 passed、全量 **202 passed** | 2026-09-20 |
| 台账补正 · cp-3 没有独立提交（cp-8c 自查） | 复盘发现本轮**不存在 `cp-r012-3` 这个 checkpoint**：cp-3 的产物（超管隐身取票 `hidden/canPublish=false` + `assert_room_role`/`assert_manager_role` 两处旁路 + `backend/tests/test_superadmin_room_access.py` + 模块实现页建页）是**并进 cp-4 的提交 `6051756`** 一起落盘的（该提交 19 个文件） | 处置：**不补 tag**（给同一个提交挂两个语义不同的 checkpoint 会让台账失真）；本行即为补正记录，cp 序列以 §本表实际提交为准（cp-2 → cp-4 → cp-4b → cp-5 …）。代码与用例均在分支上，无内容缺失 | 2026-09-20 |
