...
| E13 | 超管只管理、不发布 | `issue_room_token` 超管分支；`DeviceBar.superadminMode`；`useLocalDeviceState(publishDevices=false)` | 应用内取票 claims：`canPublish=false` / `canPublishData=false` / `roomAdmin` 非真；真机交流页：控制坞只有「管理视角 · 隐身」+「离开 / 结束房间」，**无**麦克风 / 摄像头 / 共享 / 举手控件（截图 `%TEMP%\lg_r012\cp7-superadmin-room.png`） | **通过** |
...
| 降级复测 | `global.css` 第 2063 行 `@media (prefers-reduced-motion: reduce) { .gc-drawer { transition: none; } }` | 规则存在且只关位移不早退 | **静态核对通过**；浏览器强制模拟（`emulateMedia`）工具不支持，未做（见 §6 未闭合 ②） |
...
| # | 未闭合项 | 原因 | 建议处置 |
| --- | --- | --- | --- |
| ① | 超管隐身的**双浏览器交叉验证**（另一账号的成员列表 / 舞台 / 参与者列表看不到超管；满员房实测进入） | 本机工具浏览器只有单会话；且工具浏览器内 LiveKit 媒体连接未建立（徽标「未连接」，控制台无 JS 错误） | 建议你按教学页 §3 用两个浏览器各跑一次（3 分钟）；若判 C，登记为人工剧本项；若判 D，需回阶段 0 |
| ② | `prefers-reduced-motion` 的**浏览器强制模拟** | 工具不支持 `emulateMedia` | 建议并入上条一起人工看一次（DevTools → Rendering → Emulate prefers-reduced-motion）；CSS 规则已存在并核对 |
