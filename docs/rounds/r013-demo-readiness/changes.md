...
| 2026-09-20 | cp-23 | 交付说明**正文改为作业交付物四项**（§1 源代码或 Git 仓库链接 / §2 设计说明+README+环境变量示例 / §3 测试或冒烟脚本运行说明 / §4 使用的 AI 工具与模型列表）：原「起服务与演示」并入 §1、原「设计说明导读」并入 §2、原「证据索引」并入 §3；原「评审 15 分钟复核路径」「提交口径」降为附录 A / B；根 `交付说明-题目A.docx` 同源重出（v6，`docx_validate` ok）；根 README、`docs/README.md`、roadmap §1 的指路描述同步 | 你「将交付说明主要内容改为四项」 |
...
| 2026-09-20 | cp-21 | 交付说明**简化定稿**：只留交付物落点 / 提交口径 / 起服务与演示 / 设计说明导读 / 证据索引五节（交付物表去掉状态列、删去核对表与边界表）；`ai-tools-and-models.md` 同步清掉占位行与否定式措辞 | 你「简化，不要出现任何未完成项和问题项」 |
| 2026-09-20 | cp-20 | 交付用 docx 导出件落到仓库根 `交付说明-题目A.docx`（由 `docs/00-project/global-delivery.md` 同源导出：标题页含姓名/题目/投入小时数/日期，正文 8 节 + 7 张表，页脚页码；`docx_validate` ok、结构与关键串逐项核过） | 你「交付写一份 docx 到项目根」 |
| 2026-09-20 | cp-19 | 交付说明页 v3：补**部署声明**（未用 Docker / 未做公网部署 + 实测证据：`git ls-files` 无 docker 文件、`docker --version` 不存在、WSL 无发行版；起服务与交付形态写明）；`ai-tools-and-models.md` §3 同步 | 你「声明部署或 docker 也没做」 |
