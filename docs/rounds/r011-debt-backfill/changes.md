...
| cp-8（延迟收敛，2026-09-20） | **164 passed**（51.68s，后端未改逻辑） | **PASS 47/47** | exit 0 | exit 0（4.00s） |
...
## 3.6 cp-8 逐处改动（响应延迟收敛，L0/L1）

见 `redirect-03.md` §3 的完整表；要点：焦点申请轮询 8s→3s、待批申请兜底 5s→3s、等待室 5s→2s、名册兜底 30s→10s、`refetchOnWindowFocus` false→true、房间列表新增 5s 轮询、README/AGENTS/dev.bat 统一「后端 127.0.0.1:8000 / 前端 localhost:5173」并说明 `localhost:8000` 每次多约 2 秒的成因（实测 2070ms vs 6ms）。
...
## 6. 分支与交接（2026-09-20 cp-8 事故与归位，如实记录）

**事实**：cp-8 的第一条提交（`d54ebfa`）**落错了分支** —— 提交时工作树已在 `req/r012-superadmin-console` 上（该分支由另一会话在你这边并行推进，2026-09-20 从 r011 的 `ab8edb5` 开出，含它自己的 `cp-r012-1` = `0192e25`），而我没有在写 git 前重核 `HEAD`。

**归位三步**（**未 amend、未 rebase、未改任何已 review 内容**）：
1. 先把 `req/r011-debt-backfill` 快进到 `d54ebfa`；随即发现它**把 r012 的 `cp-r012-1` 一起带进了 r011**（因为那条 cp-8 是接在 `0192e25` 之上的）。
2. 把 `req/r011-debt-backfill` 指针退回 `ab8edb5`（`git branch -f`，只移指针），在 r011 上**重做**该提交：`git cherry-pick d54ebfa` → 新提交 `a4d5541`（唯一冲突是需求索引里 r012 那行，已按「r011 分支不带 r012 行」解决）。
3. `cp-r011-8` 标签移到 `a4d5541`；错落的那条留档为附注标签 **`cp-r011-8-misfiled` = `d54ebfa`**（不删除，便于事后核对）。`req/r012-superadmin-console` 指针已回到它自己的 `0192e25`（`cp-r012-1` 未受影响）。

**都没做过什么**：没有 amend、没有 rebase、没有修改 r012 的 `cp-r012-1` 内容、没有改动任何已 review 的轮次页。

**合并顺序的后果**：r012 分支的祖先链是 `ab8edb5`（**不含** cp-8），因此合并顺序必须是 **先 r011 → 后 r012**（main 先拿到 cp-8；r012 合并时不会覆盖它）。r012 分支自身若要看到延迟收敛，合并 main 后自然带上。

**教训（写给我自己）**：本机常有多会话/多 agent 并行，工作树可能已被别人切走 —— 每次 git 写操作前必须 `git rev-parse --abbrev-ref HEAD` 断言当前分支，不凭上一次的认知。
