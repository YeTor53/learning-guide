...
| E11 | `components/QuoteLine.tsx` + `content/philosophy.ts`（46 条 / 7 组） | 按 redirect-02 批复改为**替换 A 段原文 + 场景化随机**：实测首页 hero「参差多态…——罗素」、登录页「在隆冬…——加缪」、注册页「知人者智…——老子」、404「当你凝视深渊…——尼采」、新建房间页「真理是时间的女儿…——培根」、成员抽屉空态「学而不思则罔…——孔子」；旧解释/抒情句与 404 内部话均已不在；**首页连刷 8 次得 8 种**（随机有效） | **通过** |
| E12 | 各页空态与入口 | 被删引导文字处均有可见入口：首页空态主按钮「创建房间」、请先登录「去登录」、筛选空态「清空筛选」、等待页 `WaitTimeline`、顶栏登录入口、404「回房间列表」（DOM 断言 + 截图） | **通过** |
