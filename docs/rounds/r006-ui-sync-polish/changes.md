...
| cp-r006-5 | 收官（门禁 / r004 欠账 / 文档回填 / review 定稿） | **完成 2026-09-19** | 见收官提交 | E9/E10 通过（§4） |
| cp-r006-6 | 哲学语句铺到各处解释性文案旁（按你的补充；**已被 cp-7 取代**） | **完成 2026-09-19** | 见收尾提交 | 见 §3.6 |
| cp-r006-7 | 按 redirect-02 批复重做：**替换**A 段原文 + 场景化随机 + 池子 46 条分组 + 视觉引导代偿 | **完成 2026-09-19** | 见 cp-7 提交 | E11/E12 实测（§3.7） |
...
| cp-1/2 后 | 111 passed | —— | exit 0 | exit 0 |
| cp-6 / 收官 | **111 passed** | **PASS 40/40** | exit 0 | exit 0 |
...
实测（Playwright，1440×900；同一天内多次读取稳定）：

| 位置 | 实际显示 |
| --- | --- |
| 首页 hero（`home-hero`） | 「吾生也有涯，而知也无涯。—— 庄子」 |
| 未登录侧边栏（`guest`） | 「在隆冬，我终于知道，我身上有一个不可战胜的夏天。—— 加缪」 |
| 新建房间页（`new-room`） | 「吾生也有涯，而知也无涯。—— 庄子」 |
| 抽屉「讨论」空态（`chat-empty`） | 「认识你自己。—— 德尔斐神庙箴言」 |
| 抽屉「成员」空态（`members-all` / `members-empty`） | 「参差多态，乃是幸福的本源。—— 罗素」 |
| 交流页空态（`stage-empty`） | 已接线；实机在「一个人且已有摄像头占位轨」时不出现（该空态本身就不会渲染），**如实记录**：房间空到没有任何轨道时才可见 |

> 口径核对：所有功能文本（「我的房间」需要登录后才能查看 / 你将成为房主… / 还没有人发言 / 此刻没有人在房间里…）**均未被替换**，语句是新增的一行。

### 4.1 cp-5（收官）

- **r004 遗留：竖屏横条量测**（两人同时在线）：720×1024 → `live-rail-strip`、单行、焦点格 **672×378**；900×600 → `live-rail-single`、焦点格 **622×350**；两档 `documentElement.scrollWidth <= window.innerWidth`（无横向溢出）✓
- **用例健壮性修正**（测试欠账，非产品变更）：`test_list_rooms_filters_and_aggregates` 原断言 `total == len(items)`，而演示库活跃房间已超列表默认页长（实测 total 26 / 返回 20）→ 改为显式 `limit=100` + `total >= len(items)` + 全为 active。修后 `pytest` **111 passed**（连跑稳定）。
- 门禁：`pytest` 111 / `smoke` **PASS 40/40** / `tsc` exit 0 / `build` exit 0。

### 3.7 cp-7（名言按 redirect-02 批复重做，E11/E12）

实测（Playwright，1440×900；每次进入页面随机）：

| 位置 | 实测 |
| --- | --- |
| 首页 hero | 「参差多态，乃是幸福的本源。—— 罗素」；**旧解释文已不在**（「面向一门学习主题」= False）；空态仍有就地「创建房间」按钮 |
| 登录页 | 「在隆冬，我终于知道，我身上有一个不可战胜的夏天。—— 加缪」；旧抒情句已删 |
| 注册页 | 「知人者智，自知者明。—— 老子」；旧抒情句已删 |
| 404 页 | 「当你凝视深渊时，深渊也在凝视你。—— 尼采」+「人生天地间，忽如远行客。」；**内部话「房间管理页已在 r002 移除」已删** |
| 新建房间页 | 「真理是时间的女儿。—— 培根」；旧「负责批准加入申请、结束房间」说明已删 |
| 成员抽屉空态 | 「学而不思则罔，思而不学则殆。—— 孔子」；状态事实「全部成员都在房间里」**保留** |
| **随机性** | 首页连刷 8 次，hero 那句出现 **8 种**不同名言（场景组内随机） |
| E12 代偿 | 各被删文字处均留有可见入口（首页空态主按钮 / 去登录 / 清空筛选 / 等待页进度 / 顶栏登录 / 回房间列表） |

门禁：`tsc` exit 0 / `build` exit 0。
