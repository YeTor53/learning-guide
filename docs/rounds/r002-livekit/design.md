---
title: r002 设计：实时房间（M2）
description: r002 的契约面清单、数据模型与迁移、接口与函数级实现、实时层设计、治理动作、视觉与教学契约、回退与风险。
type: concept
status: closed
owner: 陀梓皓
updated: 2026-09-18（2026-09-19 补 front matter，正文未改）
---
...
| 页面与路由 | 新增 `/rooms/:id/live`（交流页）与 `/rooms/:id/wait`（等待页）；`/rooms/:id` 口径改为房间管理页 | 路由、页面职责与情绪取向变化 |
...
| 9 | **页面职责三分**：房间管理页（治理）+ 交流页（专注交流）+ 等待页（等待获批） | 三种场景的诉求完全不同：治理要信息全、交流会要打扰少、等待要情绪安抚；混在一页里必然两头不讨好（r002 原 F-13 就是「一页两用」） | 继续一页两用：进房页既要报错又要等待，观感与逻辑都别扭 |
| 10 | **两页情绪靠令牌而非新体系**：专注感/温暖感各一组 CSS 参数，单点可调 | 不引第三方动效或 UI 库、不换设计体系（ADR-0008），改情绪只改参数区 | 引动画库/换主题：与离线演示和体积约束冲突，且交付要求禁用默认页面 |
...
| --- | --- | --- | --- | --- | --- |
| redirect-04 | 2026-09-18 | 范围变更（按 L3 看待） | 页面职责三分（房间管理页 / 交流页 / 等待页）+ 两种情绪取向（专注感 / 温暖感）；F-13 改写、新增 F-16/F-17；cp 拆为 2/3/4/5 | `redirect-04.md`（confirmed-B） | 本次文档提交 |
