...
| 单号 | 类型 | 级别 | 结论 | 落地 SHA |
| --- | --- | --- | --- | --- |
| redirect-04 | 重定向（范围变更） | 按 L3 看待 | confirmed-B：页面职责三分 + 两种情绪取向落地文档（术语表/风格指南/需求单/功能页/实现页）；redirect-02 的 G1 等候态并入本单 | 见本次文档提交 |
| redirect-01 | 重定向（范围变更） | 按 L3 看待 | confirmed-C：断线重连 + 设备状态保持并入 r002；C-1/C-2 口径修正已落地文档 | 见本次文档提交 |
| （CR 待实现期追加） | | | | |
