...
| 单号 | 类型 | 触发 | 定性 | 状态 | 结论 |
| --- | --- | --- | --- | --- | --- |
| `redirect-01.md` | 用户新意见（断线重连与状态保持） | 设计待批阶段，用户问「断线重连，状态保持有码」 | C（后续工作）+ 其中「连接层/设备层」属提前做＝范围变更（按 L3 看待） | **proposed（待你一次批复）** | 建议：断线重连（连接层）+ 设备状态保持（设备层）进 r002；业务状态恢复留 M3/M5 ①；另修正 C-1（断线归因改用 SDK reason）、C-2（同账号双开的口径） |
