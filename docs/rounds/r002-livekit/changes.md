...
| `redirect-06.md` | 用户新意见（管理页是否该移除） | 实现期，你提「思考管理页是否该移除，太重型了」 | B（**保留并降型**为「门口页」；先成文待批） | **proposed（待批复）** | 盘点：该页五项内容里三项与房内抽屉重叠（重型的来源），但「只读回访」与「未在册者落脚点」删不掉 → 四选项对比，建议 B：去掉成员治理与申请处理、保留介绍与入口与回访；成本后端 0、前端 1 页、文档属 L3 |
