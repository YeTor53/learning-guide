...
1. **动效暂未验到**：无头 Chromium **默认就是 `prefers-reduced-motion: reduce`**，我的降级逻辑正确地把动效归零（这解释了第一轮「动画数=0」）。用 `page.emulate_media(reduced_motion='no-preference')` 打开后，我的采样窗口又**没对上变化瞬间**（60 帧采样期间第三人还没进到舞台）→ FLIP 是否真的播放，**下一轮补测**（方法：先在页面里装 rAF 采样器，再触发变化）。
