...
## 5c. 追加实测：离线语音回归（2026-09-20，cp-6）

在「检查一遍」之后追加的一次**零配额**真机回归（脚本已入库 `frontend/scripts/verify-transcription.py`，素材为本机 TTS 合成语音）：

- **音频路径有硬数字了**（此前只有「worker 只给有音频轨的人开会话」的间接证据）：本端采集 RMS 峰值 0.214 / 0.273；**对端**订阅 3 条音频轨，4 秒收 **+35~43 KB**、`totalAudioEnergy` **+0.35~1.09**。
- 转写链路同前（4 人、48~53 条、全端「开启」、worker 开 4 个会话）；该房 **361 次上报全部 201 → 库内 96 行**（幂等成立，冗余 3.76×）；该房请求**无 4xx/5xx**。
- **两条测量教训**（已写进模块页 §6，后续轮次别再踩）：① 本机没有可用音频输出设备时，浏览器 AudioContext 侧量远端音频恒为 0，应改读 **WebRTC 接收统计**（dev 句柄 `window.__lgRoom`）；② **不能连上就测**——本机首连常先超时换区，实测最慢 **24.2s**，脚本已加「等 `state === 'connected'` 且远端人数达标」的门。
- 本次未改变 E1~E14 的任何结论；未闭合 ③（假 STT 下渐进样本）依旧，另新增说明：**假 STT 下「音频内容对识别结果无影响」是设计使然**，识别质量只能由真 STT 回归覆盖。
