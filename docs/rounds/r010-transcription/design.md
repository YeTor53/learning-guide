...
| 位置 | 原文 |
| --- | --- |
| 进房告知条 | 「本房间会把说的话转成文字，供大家与纪要使用。音频会经 LiveKit 云与识别服务商处理，不留存录音；关掉麦克风即不参与转写。」+ 按钮「关掉我的麦克风」/「知道了」（换轨后口径：不再提供「关本端上传」的开关） |
| 控制坞状态（只读） | 「转写：开启」/「转写：未开启」（判据＝房内有没有 `agent-*`；是文案不是按钮，避免「点了没反应」） |
