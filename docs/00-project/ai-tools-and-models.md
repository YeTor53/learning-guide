...
description: 本仓用到的编程助手、对话模型与推理服务清单，以及实际投入小时数（作业交付物）。
type: reference
status: draft
owner: 陀梓皓
updated: 2026-09-20
...
<!-- overview -->
作业原文要求「提交时列出工具与模型；你需能讲清架构、关键决策与验证方式」。以下为本仓实际使用的工具与模型清单。
...
- 提交时注明：所选题目 = **A**；**实际投入小时数 ≈ 12 小时**（2026-09-20 本人确认口径；含需求与设计文档、后端与数据库、前端、测试与文档回填的合计）。
- 运行形态：开发用本机 `dev.bat` 两个进程（uvicorn + Vite）；交付与演示形态 = uvicorn 托管前端 `dist` 的单进程同源（`.env` 的 `APP_ENV=demo`），无需容器。
