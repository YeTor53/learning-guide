...
| **工具栏位置回退 + 首屏引导 + 主题控件风格化 + 主题扩容 + 侧边栏默认收起**（你 2026-09-19 提出） | 界面口径（你已给批复：`1 2 1 1 1` + 工具栏换回 + 不加压 hero） | **进行中（r007）**：实测筛选条已在首屏但房间列表仍在折线下方 10~142px、hero 与列表间 562~764px 空档；主题筛选是原生 `<select>`（未风格化）；主题清单被 DB CHECK 锁死需迁移 006。需求单/design 已落 | r007（`req/r007-topic-and-scrollhint`） |
