...
> **M3（r004）状态回填（2026-09-20，r009.5 补）**：**已完成**。r004 实现（群聊落库 / 举手 / 焦点发言 / 屏幕共享与「焦点×共享」优先级 + 两项界面缺陷 + 还债）由你于 2026-09-19 合并（`daf7696`）并打 `round-r004-done`；M3 之后的增量全部走补轮：**r005 容量口径**（ADR-0016）、**r006 界面同步**、**r007 主题与首屏**、**r008 纪要 + 限时邀请**（作业必做补全）、**r009 焦点系统重做**（ADR-0021）——均**未合入 `main`**，合并顺序 `r005→r006→r007→r008→r009→r010→r009.5`。
>
> **M4（会后产出 · 交付）状态回填（2026-09-20，r009.5 补）**：**部分提前落地**。LLM 纪要链路（r008：表 + 服务 + 路由 + 纪要页 + 真机 6.0 秒 / 632 字 / 反编造）已实现，冒烟 46 步；`README` / `.env.example` / 设计说明 / 教学页持续维护。**未做**：打包提交（zip / GitHub / npm 口径 = P11 暂缓）、AI 工具与模型清单（`docs/00-project/ai-tools-and-models.md` 待你确认）、服务端录制转写（转写改走路径 B，见 ADR-0022）。
>
> **M5（加分项）**：0/5 完成（断线重连为部分：状态落库 + 重连刷新 + 30 秒兜底已做，物理断网演练 E18c 待你执行）；转录写（r010）属本清单第 ② 项，本轮起做。
...
10. **r010 进行中（2026-09-20）**：语音转文字并入讨论流 + STT 获取方案。阶段 1 文档已落（需求单 + 函数级设计 + ADR-0022 + 调研 01/02），**cp-1 起未开工**；真机转写取证等 `STT_*` 三键填值。
11. **r009.5 当前轮（2026-09-20）**：r008/r009 欠账补正（台账 · tag · 审查回填 · 纠错 · 取证 · 教学页），**只修欠账不加功能**；见 `docs/00-requirements/r009.5-debt-backfill.md`。合并顺序：**r005 → r006 → r007 → r008 → r009 → r010 → r009.5**。
...
| 语音转文字（默认开启 + 并入文字对话 + 含管理信息）+ STT 获取方案设计 | 你 2026-09-19 追加 | **已开轮 r010**（2026-09-20）：澄清单 Q1~Q12 已批、调研 01/02 + 函数级设计 + ADR-0022 已落（`.env.example` 三键已补）；cp-1 起未开工 | r010（`req/r010-transcription`，阶段 1 文档已落） |
