...
4. **r004（M3）当前：实现完成、待合并（2026-09-19）**。阶段 2 全部落地（cp-2~cp-7）：后端迁移 `004` + 三条 service + 8 条路由、前端 Data Channel 加速层与四个状态 hook、`stageLayout` 单一布局事实源 + 讨论区 + 抽屉双 tab + 举手/焦点/共享、两项界面缺陷与还债；门禁 `pytest` 105 passed / `smoke` PASS 36/36 / `tsc`+`build` exit 0；取证 8 人档、720×1024 竖屏与 900×600 横条量测齐全（E18b 如实标未做、E18c 待你真机演练）。**合并后由人打 `round-r004-done`**。范围 = 群聊落库 + 举手 + 焦点发言 + 屏幕共享与「焦点 × 共享」优先级，另并入两项界面缺陷（首页筛选条上移、侧边栏竖屏）与还债（smoke 四步 + 三处实测留痕）。入口：`docs/00-requirements/r004-room-extras.md` + `docs/rounds/r004-room-extras/design.md`（§0 导航）。
...
| **房间卡显示 `8/8` 却仍可进人**（你 2026-09-19 报：「毛病？按真实房间人数限制实现」） | 口径不一致（A 实现偏差）+ 设计变更（L3） | **已登记，待拍板（r004 `redirect-02`）**：三比结论 —— 列表卡用「在册/容量」（`RoomCard.tsx` L66/162），状态条用「在场/容量」（`RoomLivePage.tsx` L336），准入用「在场」且**查询失败降级放行**（ADR-0012 `D5` 原文）；修法 = 三处统一到「在场」+ 失败改 fail-closed（需批 L3）+ 取票短超时/缓存；建议新补轮 `r005-fix-capacity`（4 cp） | r004 `redirect-02` |
