...
| ~~麦克风图标不同步~~（**已修，2026-09-19，r006**） | **A 实现偏差**（r002 已批准的验收点没做到：图标挂在「有没有摄像头」上，没读麦克风状态） | **已复现并定位**：双浏览器实测 A 未静音时格子已带静音图标、静音后图标不变（B 端 SDK 里 `micEnabled` 已正确同步）→ 纯前端呈现缺陷；确认单 `docs/rounds/r006-fix-mic-badge/redirect-01.md`（Q1~Q4 待批） | r006（`req/r006-fix-mic-badge`，从 r005 分支拉） |
| ~~两端人数显示不一致~~（**已修，2026-09-19，r006**） | **A 实现偏差（r005 引入）**：状态条改成「在册」后变成「页面加载快照」，只有本端动作才刷新；别端动作不通知本端 | **已复现**：A（房主）界面批准后 5 秒 → A `3/8`、B 仍 `2/8`（库真值 3），B 刷新后才一致；待批徽标同样滞后（r002 起的老问题）。确认单同页 `docs/rounds/r006-fix-mic-badge/redirect-01.md` §8（Q5~Q7 待批） | r006（同上） |
