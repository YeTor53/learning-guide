...
| **首页筛选工具栏落在首屏之外**（你 2026-09-18 提「toolbar 太下了」） | 界面口径未定（不是 A 实现偏差：功能页 §4.1 与 ADR-0007/0008 都没写「筛选条须在首屏可见」） | **r002 不做，登记待你拍板**。实测（你正在跑的 dev 页面 `localhost:5173`，视口高 566px）：顶栏 `top:0`、高 61（贴顶正常）；首页 hero 区高 **498px**，筛选 `.toolbar` 顶边在 **639px** → 首屏完全看不到，需下滚约一屏。候选修法（选一个我再动）：① 压 hero 纵向留白（kicker/副标题/统计间距）② 把 `.toolbar` 提到 hero 之上 ③ 小视口（高度 < 700px）自动收窄 hero ④ `.toolbar` 改 sticky 吸顶 |
