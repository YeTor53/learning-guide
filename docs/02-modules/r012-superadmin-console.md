...
| 日期 | 轮次 | 变更 | 依据 |
| --- | --- | --- | --- |
| 2026-09-20 | r013 `cp-6` | 大屏面板拆成 `GlobalChatPanel`（`variant=drawer|embedded`）：常规页仍是顶栏抽屉（`GlobalChatDrawer` 只剩壳与 Esc），**交流页新增第 4 个 tab「大屏」**（替代原「交流页不挂」口径）；数据源仍是单例 `useGlobalChat` + SSE | r013 需求单 E10；真机 4 tab + 跨端 0.5 秒 |
