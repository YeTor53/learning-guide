...
| 8.5a | 准入判定 vs 准入执行（前端门） | 交流页连接前**先取票**（`POST /token`，服务端校验成员身份与房间状态），取不到就重定向到等待页——这层是**UX 门 + 「经我们后端路径」的门**，能让名单/成员状态/房间状态全部生效；但**不是安全边界**：客户端拿一张仍在有效期的旧票即可绕开页面直连 LiveKit，因此旧票问题只能由平台侧（Cloud `revoke_token_ts`）或短 TTL（自建）解决（ADR-0011 条 10a） |
