...
1. **声波（E11）**：**r009.5 实测（2026-09-20）**已定位到正确的链路——`.live-level` 是容器，里面是 **3 个 `<i>` 竖条**（单条高 6px，随音量加 `.on` 类），刷新节奏令牌 `--mic-pulse-ms` = **320ms**；本轮取证脚本按纪律**不发布测试音源**，故 `on` 计数恒为 0、**数值序列仍未取**（需真人说话或发布测试音源）→ 顺延 r011，见 `docs/rounds/r009-focus-system/review.md` 未闭合 ①。
