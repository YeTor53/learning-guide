...
- 本机跑通：房间列表 → 申请加入 → 房主在**交流页抽屉**里批准 → 申请人**自动进入**音视频房间 → 房主移出/授权 → 房主在**控制坞**点「结束房间」收尾
- 三条页面：`/`（房间列表）、`/rooms/:id/live`（交流页）、`/rooms/:id/wait`（等待室）
- 一键启动（Windows）：`dev.bat` 起后端 `:8000` + 前端 `:5173`；`dev.bat check` 只做自检；`dev.bat stop` 停掉两者（实现与坑见 `docs/rounds/r003-end-room-entry/redirect-01.md`）
