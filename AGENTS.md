...
```bash
pytest backend/tests -q                                  # 202 项（2026-09-20 cp-8b 实测；数字会漂，以命令输出为准）
npx tsc --noEmit --project frontend                      # 前端类型
python backend/scripts/smoke.py --base-url http://127.0.0.1:8000   # PASS 59/59（2026-09-20 cp-8b 实测，含 r012 超管/后台/大屏 11 步 + SSE 不占连接池 1 步）
```
